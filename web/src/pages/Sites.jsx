      ...
      extra={
        <Space size={10} wrap>
          ...
          <Button
            ...
            size="large"
            onClick={() => {
              fetch('/api/email-config/test', { method: 'POST', headers: authHeaders(true) })
          ...
          {/* TEST: trigger global check */}
          <Button
            icon={<ThunderboltOutlined />}
            size="large"
            onClick={() => {
              fetch('/api/schedule-config/trigger', { method: 'POST', headers: authHeaders(true) })
                .then((r) => { if (!r.ok) throw new Error('请求失败'); return r.json() })
                .then(() => message.success('已触发全局检测'))
