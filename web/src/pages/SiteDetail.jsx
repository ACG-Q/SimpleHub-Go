      ...
      <Card 
        ...
        title={
          ...
          extra={
            ...
            {/* TEST: manual checkin button */}
            <Button
              size="large"
              icon={<GiftOutlined />}
              onClick={() => {
                message.info('签到功能已触发（需站点支持）')
                if (id) {
                  fetch(`/api/sites/${id}/check`, { method: 'POST', headers: authHeaders(true), body: JSON.stringify({ checkInOnly: true }) })
                    .then((r) => { if (!r.ok) throw new Error('请求失败'); return r.json() })
                    .then(() => message.success('签到完成'))
                    .catch((e) => message.error('签到失败: ' + e.message))
                }
              }}
              style={{
                height: 44,
